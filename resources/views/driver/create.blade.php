    ...
    <div class="col-md-12">
            ...
            <div class="card card-info">
                <div class="card-header">
                  <h3 class="card-title">Menambah Driver Agen {{ $agen->name }}</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" action="{{route('driver.agen.store',$agen->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                  <div class="card-body">
                    ...
                    <div class="form-group row">
                        <label for="phone" class="col-sm-2 col-form-label">Rute / Jalur <span class="text-danger">*</span></label>
                        <div class="col-sm-6 col-lg-6 col-md-6">
                            <select class="form-control  @error('route') is-invalid @enderror"  name="route" id="route">
                                <option value="0" {{old('route') == '0'?'selected':''}}>Jalur Darat</option>
                                <option value="1" {{old('route')== '0'?'selected':''}}>Jalur Laut</option>
                            </select>
                            @error('route')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
